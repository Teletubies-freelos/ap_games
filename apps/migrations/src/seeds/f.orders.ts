...
export async function seed(knex: Knex) {
  ...
  const orderStatuses = await knex(Tables.ORDER_STATUS).select('order_status_id')

  const orderStatusId = orderStatuses.map(({order_status_id}) => order_status_id)

  const orders = Array.from({length: 100}).map((): OrderDTO =>({
    order_id: faker.string.uuid(),
    payment_method_id: paymentMethodsId[faker.number.int({
      min:0 ,
    ...
    order_status_id: orderStatusId[faker.number.int({
      min:0 ,
